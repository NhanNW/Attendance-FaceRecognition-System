...
const config = {
  connectionString:
    "Driver={ODBC Driver 17 for SQL Server};Server=THANHNHAN\\SQLEXPRESS;Database=HeThongChamCong;Trusted_Connection=Yes;"
};
...
app.post("/api/register", async (req, res) => {
  try {
    const { employeeID, fullName, dob, gender, phone, address, position, department, password, role } = req.body;

    if (!employeeID || !fullName || !dob || !gender || !position || !department || !password || !role) {
    ...
    await pool.request()
      ...
      .query(`
        ...
        (EmployeeID, E_FullName, E_DOB, E_Gender, E_Phone, E_Address, E_FaceData, E_Position, Status, DepartmentID)
        VALUES (@EmployeeID, @E_FullName, @E_DOB, @E_Gender, @E_Phone, @E_Address, @E_FaceData, @E_Position, @Status, @DepartmentID)
      `);
